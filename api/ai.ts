...
export async function GET(request: Request) {
  ...
  if (url.pathname.endsWith("/health")) {
    return json({ ok: true, service: "irenx-omniroute-core", ...observabilitySnapshot() });
  }
  ...
  return json({
    service: "IRENX OmniRoute Core Router V2",
    endpoints: ["POST /api/ai", "GET /api/ai/health", "GET /api/ai/route?prompt=..."]
  });
}
